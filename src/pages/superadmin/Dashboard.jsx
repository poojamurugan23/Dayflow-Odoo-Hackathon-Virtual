...
export function SuperAdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch notifications instead of pending leaves and attendance for superadmin
      const resNotif = await fetch(`${API_BASE}/api/data/notifications`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (resNotif.ok) {
    ...
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-6 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-[#171923]">Global Access Mode</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Absent Today', value: (metrics.totalEmployees - metrics.presentToday - metrics.approvedLeaves).toString(), icon: UserX, color: 'red' },
          { label: 'Time Off / Leaves', value: metrics.approvedLeaves.toString(), icon: Calendar, color: 'amber' }
        ].map(card => {
